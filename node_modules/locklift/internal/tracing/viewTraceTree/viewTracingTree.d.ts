...
declare type NameAndType<T extends string = string> = {
    type: TraceType;
    name: T;
    contract?: Addressable;
};
declare type EventNames<Abi> = DecodedEventWithTransaction<Abi, AbiEventName<Abi>>["event"];
declare type MethodParams<C extends Contract<any>, N extends keyof C["methods"]> = Parameters<C["methods"][N]>[0];
declare type EventParams<Abi, N extends string> = Extract<DecodedEventWithTransaction<Abi, AbiEventName<Abi>>, {
    event: N;
}>["data"];
