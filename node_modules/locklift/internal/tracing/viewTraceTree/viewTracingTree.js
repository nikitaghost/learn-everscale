...
class ViewTracingTree {
    ...
    findByTypeWithFullData = (params) => this._findByType(params, [this.viewTraceTree]);
    _findByType = ({ type, name, contract }, tree) => {
        if (tree.length === 0) {
            return [];
        }
        ...
        for (const trace of tree) {
            ...
            if (trace.outTraces.length > 0) {
                return [...matchedMethods, ...this._findByType({ name, type, contract }, trace.outTraces)];
            }
        }
    ...
    _beautyPrint = (viewTrace, offset, contracts) => {
        let traces = "";
